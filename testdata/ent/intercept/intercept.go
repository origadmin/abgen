...
import (
	...
	"github.com/origadmin/abgen/testdata/ent"
...
// The SoftDeleteFunc type is an adapter to allow the use of ordinary function as a Querier.
type SoftDeleteFunc func(context.Context, *ent.SoftDeleteQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f SoftDeleteFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.SoftDeleteQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.SoftDeleteQuery", q)
}

// The TraverseSoftDelete type is an adapter to allow the use of ordinary function as Traverser.
type TraverseSoftDelete func(context.Context, *ent.SoftDeleteQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseSoftDelete) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseSoftDelete) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.SoftDeleteQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.SoftDeleteQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.SoftDeleteQuery:
		return &query[*ent.SoftDeleteQuery, predicate.SoftDelete, softdelete.OrderOption]{typ: ent.TypeSoftDelete, tq: q}, nil
	case *ent.UserQuery:
